    ...
    const waitForStore = () => {
        ...
        if (store && !store.createMagicLink) {
            ...
            store.createMagicLink = async function(email, redirectUrl = window.location.href) {
                ...
                try {
                    // Check if account method exists and log for debugging
                    if (!this._appwrite || !this._appwrite.account) {
                        console.error('[Indux Appwrite Auth] Account instance not available', {
                            hasAppwrite: !!this._appwrite,
                            hasAccount: !!(this._appwrite && this._appwrite.account)
                        });
                    ...
                    const availableMethods = Object.getOwnPropertyNames(Object.getPrototypeOf(account)).filter(
                        name => typeof account[name] === 'function' && name.startsWith('create')
                    );
                    console.log('[Indux Appwrite Auth] Available Account methods:', availableMethods);
                    ...
                    console.error('[Indux Appwrite Auth] Magic link methods not found. Available create methods:', availableMethods);
                    return { 
                        success: false, 
                        error: 'Magic link method not available. Please ensure you are using the latest Appwrite SDK.' 
                    };
                } catch (error) {
                    console.error('[Indux Appwrite Auth] Magic link error:', error);
                ...
                try {
                    ...
                    if (this.session && this.isAnonymous) {
                        try {
                            ...
                            console.log('[Indux Appwrite Auth] Deleted anonymous session before creating magic link session');
                        } catch (deleteError) {
                    ...
                    console.error('[Indux Appwrite Auth] Magic link callback failed:', error);
...
function handleMagicLinkCallbacks() {
    ...
    window.addEventListener('indux:auth:callback:expired', async (event) => {
        ...
        console.log('[Indux Appwrite Auth] Expired magic link detected');
        
        // Check if user is already authenticated - preserve session
        let hasExistingSession = false;
        if (store.isAuthenticated && store.user) {
            hasExistingSession = true;
            console.log('[Indux Appwrite Auth] Preserving existing session for expired magic link');
    ...
    window.addEventListener('indux:auth:callback:magic', async (event) => {
        ...
        const callbackInfo = event.detail;
        console.log('[Indux Appwrite Auth] Magic link callback detected');
...
document.addEventListener('alpine:init', () => {
    try {
        ...
        console.error('[Indux Appwrite Auth] Failed to initialize magic links:', error);
    }
});
