    ...
    const waitForStore = () => {
        ...
        if (store && !store.loginOAuth) {
            ...
            store.loginOAuth = async function(provider, successUrl = window.location.href, failureUrl = window.location.href) {
                ...
                console.log('[Indux Appwrite Auth] Starting OAuth login:', {
                    provider,
                    successUrl: cleanSuccessUrl,
                    failureUrl: cleanFailureUrl,
                    originalHref: window.location.href,
                    hasAnonymousSession: this.isAnonymous
                });
                ...
                if (this.isAnonymous && this.session) {
                    try {
                        console.log('[Indux Appwrite Auth] Deleting anonymous session before OAuth');
                ...
                try {
                    ...
                    console.log('[Indux Appwrite Auth] Stored OAuth provider:', provider, 'in localStorage');
                } catch (e) {
                    // Fallback to sessionStorage if localStorage fails
                    sessionStorage.setItem('indux:oauth:provider', provider);
                    console.log('[Indux Appwrite Auth] Stored OAuth provider:', provider, 'in sessionStorage (localStorage failed)');
                }
                ...
                try {
                    ...
                    console.log('[Indux Appwrite Auth] OAuth token created:', token);
                    ...
                    console.error('[Indux Appwrite Auth] OAuth login error:', error);
...
function handleOAuthCallbacks() {
    ...
    window.addEventListener('indux:auth:callback:oauth', async (event) => {
        ...
        console.log('[Indux Appwrite Auth] OAuth callback detected');
        ...
        console.log('[Indux Appwrite Auth] Restoring OAuth provider:', storedProvider);
        if (storedProvider) {
            store._oauthProvider = storedProvider;
            console.log('[Indux Appwrite Auth] OAuth provider restored:', store._oauthProvider);
        ...
        try {
            ...
            if (store.session && store.isAnonymous) {
                try {
                    ...
                    console.log('[Indux Appwrite Auth] Deleted anonymous session before creating OAuth session');
                } catch (deleteError) {
                    console.log('[Indux Appwrite Auth] Could not delete anonymous session:', deleteError.message);
                }
            }
            ...
            try {
                ...
                console.log('[Indux Appwrite Auth] OAuth session created:', {
                    sessionProvider: session.provider,
                    storedProvider: store._oauthProvider,
                ...
                if (isProhibited) {
                    console.log('[Indux Appwrite Auth] OAuth session already exists, fetching user');
                    store.user = await store._appwrite.account.get();
                    try {
                    ...
                    console.log('[Indux Appwrite Auth] OAuth session restored (prohibited error):', {
                        sessionProvider: store.session?.provider,
                        storedProvider: store._oauthProvider,
                        email: store.user?.email
                    });
                } else {
            ...
            console.error('[Indux Appwrite Auth] OAuth callback failed:', error);
...
document.addEventListener('alpine:init', () => {
    try {
        ...
        console.error('[Indux Appwrite Auth] Failed to initialize OAuth:', error);
    }
});
