        ...
        if (store && !store.isRolePermanent) {
            ...
            store.getDeletedTemplateRoles = async function(teamId) {
                ...
                try {
                    ...
                    // Only log warning for unexpected errors, not for deleted teams
                    if (!e.message || !e.message.includes('could not be found')) {
                        console.warn('[Indux Appwrite Auth] Failed to load deleted roles from team preferences, falling back to localStorage:', e);
                    }
                    try {
            ...
            store.reapplyTemplateRole = async function(teamId, roleName) {
                ...
                try {
                    ...
                    console.error('[Indux Appwrite Auth] Failed to reapply template role:', error);
            ...
            store.ensureDefaultRoles = async function(teamId) {
                ...
                if (this.getDeletedTemplateRoles) {
                    try {
                        ...
                        console.warn('[Indux Appwrite Auth] Failed to load deleted roles list:', e);
                    }
                }
                ...
                try {
                    ...
                    console.error('[Indux Appwrite Auth] Failed to ensure default roles:', error);
                    return { success: false, error: error.message };
                }
            };
        } else if (!store) {
