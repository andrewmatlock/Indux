    ...
    const waitForStore = () => {
        ...
        if (store) {
            ...
            const updatePermissionCache = async function() {
                ...
                for (const perm of permissions) {
                    if (this.hasTeamPermission) {
                        ...
                        console.log('[Indux Appwrite Auth] updatePermissionCache: Cached permission', perm, '=', hasPerm);
                    } else {
                        this._permissionCache[perm] = false;
                        console.log('[Indux Appwrite Auth] updatePermissionCache: No hasTeamPermission method, cached', perm, '= false');
                    }
                }
                
                console.log('[Indux Appwrite Auth] updatePermissionCache: Final cache:', JSON.parse(JSON.stringify(this._permissionCache)));
            ...
            store.updateCurrentTeamName = async function() {
                console.log('[Indux Appwrite Auth] updateCurrentTeamName() called');
                console.log('[Indux Appwrite Auth] currentTeam:', this.currentTeam);
                console.log('[Indux Appwrite Auth] updateTeamNameInput:', this.updateTeamNameInput);
                
                if (!this.currentTeam || !this.currentTeam.$id) {
                    console.log('[Indux Appwrite Auth] No team selected');
                ...
                console.log('[Indux Appwrite Auth] teamNameValue:', teamNameValue);
                if (!teamNameValue) {
                    console.log('[Indux Appwrite Auth] Team name is empty');
                    return { success: false, error: 'Team name is required' };
                }
                
                const teamId = this.currentTeam.$id;
                console.log('[Indux Appwrite Auth] Calling updateTeamName with teamId:', teamId, 'name:', teamNameValue);
                
                const result = await this.updateTeamName(teamId, teamNameValue);
                console.log('[Indux Appwrite Auth] updateTeamName result:', result);
                
                if (result.success) {
                    console.log('[Indux Appwrite Auth] Update successful, clearing input');
                    this.updateTeamNameInput = ''; // Clear input
                    console.log('[Indux Appwrite Auth] currentTeam after update:', this.currentTeam);
                    console.log('[Indux Appwrite Auth] currentTeam.name:', this.currentTeam?.name);
                    // Note: updateTeamName() already calls listTeams() and updates currentTeam
                } else {
                    console.log('[Indux Appwrite Auth] Update failed:', result.error);
                }
                return result;
            };
...
document.addEventListener('alpine:init', () => {
    try {
        ...
        console.error('[Indux Appwrite Auth] Failed to initialize teams convenience:', error);
    }
});
