        ...
        if (store && !store.createUserRole) {
            ...
            store.createUserRole = async function(teamId, roleName, permissions) {
                ...
                try {
                    ...
                    console.error('[Indux Appwrite Auth] Failed to create user role:', error);
            ...
            store.updateUserRole = async function(teamId, roleName, permissions) {
                ...
                try {
                    ...
                    console.error('[Indux Appwrite Auth] Failed to update user role:', error);
                    this.error = error.message;
                    return { success: false, error: error.message };
                } finally {
                    this.inProgress = false;
            ...
            store.renameUserRole = async function(teamId, oldRoleName, newRoleName) {
                ...
                try {
                    ...
                    try {
                        if (this.listMemberships) {
                            ...
                            if (membershipsResult && membershipsResult.success && membershipsResult.memberships) {
                                ...
                                console.log(`[Indux Appwrite Auth] Renamed role "${oldRoleName}" to "${newRoleName}" and updated ${updatedCount} membership(s)`);
                            }
                        }
                    } catch (membershipError) {
                        // Log error but don't fail the rename operation
                        console.warn('[Indux Appwrite Auth] Failed to update memberships after role rename:', membershipError);
                    }
                    ...
                    console.error('[Indux Appwrite Auth] Failed to rename user role:', error);
                    this.error = error.message;
                    return { success: false, error: error.message };
                } finally {
                    this.inProgress = false;
                ...
                try {
                    ...
                    if (isTemplateRole) {
                        ...
                        try {
                            ...
                            console.warn('[Indux Appwrite Auth] Failed to track deleted role in team preferences:', e);
                            ...
                            try {
                                ...
                                console.warn('[Indux Appwrite Auth] Failed to track deleted role in localStorage:', e2);
                            }
                        }
                    }
                    ...
                    console.error('[Indux Appwrite Auth] Failed to delete user role:', error);
                    this.error = error.message;
                    return { success: false, error: error.message };
                } finally {
                    this.inProgress = false;
...
document.addEventListener('alpine:init', () => {
    try {
        ...
        console.error('[Indux Appwrite Auth] Failed to initialize teams user roles:', error);
    }
});
