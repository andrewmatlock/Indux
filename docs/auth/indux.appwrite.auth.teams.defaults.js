        ...
        if (store && !store.isTeamImmutable) {
            ...
            store.cleanupDuplicateDefaultTeams = async function() {
                ...
                for (const teamName of allDefaultTeamNames) {
                    ...
                    if (matchingTeams.length > 1) {
                        console.log(`[Indux Appwrite Auth] Found ${matchingTeams.length} duplicate teams with name "${teamName}". Cleaning up...`);
                        ...
                        console.log(`[Indux Appwrite Auth] Keeping team ${teamToKeep.$id} (created: ${teamToKeep.$createdAt}), deleting ${teamsToDelete.length} duplicate(s)`);
                        ...
                        for (const team of teamsToDelete) {
                            try {
                                ...
                                if (this._appwrite && this._appwrite.teams) {
                                    ...
                                    cleanedCount++;
                                    console.log(`[Indux Appwrite Auth] Deleted duplicate team: ${team.$id} (${team.name})`);
                                }
                            } catch (error) {
                                const errorMsg = `Error deleting duplicate team ${team.$id}: ${error.message}`;
                                console.error(`[Indux Appwrite Auth] ${errorMsg}`);
            ...
            store.reapplyTemplateTeam = async function(teamName) {
                ...
                try {
                    ...
                    console.error('[Indux Appwrite Auth] Failed to reapply template team:', error);
...
async function ensureDefaultTeams(store) {
    ...
    if (hasPermanent) {
        for (const nameConfig of appwriteConfig.permanentTeams) {
            ...
            try {
                console.log('[Indux Appwrite Auth] Auto-creating permanent team (mandatory):', teamName);
                ...
                if (result.success) {
                    ...
                    console.log('[Indux Appwrite Auth] Permanent team created:', result.team);
                } else {
                    console.error('[Indux Appwrite Auth] Failed to create permanent team:', teamName, result.error);
                }
            } catch (error) {
                console.error('[Indux Appwrite Auth] Error creating permanent team:', teamName, error);
            }
        }
    }
    ...
    if (hasTemplate) {
        for (const nameConfig of appwriteConfig.templateTeams) {
            ...
            if (deletedTeams.includes(teamName)) {
                console.log('[Indux Appwrite Auth] Skipping deleted template team:', teamName);
                continue;
            }
            ...
            try {
                console.log('[Indux Appwrite Auth] Auto-creating template team (mandatory):', teamName);
                // Pass empty array to use creatorRole logic from config
                const result = await store.createTeam(null, teamName, []);
                
                if (result.success) {
                    createdTeams.push(result.team);
                    console.log('[Indux Appwrite Auth] Template team created:', result.team);
                } else {
                    console.error('[Indux Appwrite Auth] Failed to create template team:', teamName, result.error);
                }
            } catch (error) {
                console.error('[Indux Appwrite Auth] Error creating template team:', teamName, error);
            }
        }
    }
...
document.addEventListener('alpine:init', () => {
    try {
        ...
        console.error('[Indux Appwrite Auth] Failed to initialize teams defaults:', error);
    }
});
