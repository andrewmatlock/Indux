...
async function processExistingCodeBlocks() {
    try {
        ...
        // Exclude elements with frame class or that are inside asides
        const codeBlocks = document.querySelectorAll('pre > code:not(.hljs):not([data-highlighted="yes"]):not(.frame)');
        
        for (const codeBlock of codeBlocks) {
            try {
                // Skip if this is inside an aside element (likely frame content)
                if (codeBlock.closest('aside')) {
                    continue;
                }
    ...
    class XCodeElement extends HTMLElement {
        ...
        extractContent() {
            ...
            if (this.children.length > 0) {
                ...
                // For HTML content, normalize indentation
