/*! Indux Collections 1.1.0 - MIT License */
...
async function initializeCollectionsPlugin() {
    ...
    async function loadFromAPI(collection) {
        try {
            const url = new URL(interpolateEnvVars(collection.url));
            
            // Add query parameters
            if (collection.params) {
                Object.entries(collection.params).forEach(([key, value]) => {
            ...
            if (collection.headers) {
                Object.entries(collection.headers).forEach(([key, value]) => {
            ...
            if (collection.transform) {
                data = getNestedValue(data, collection.transform);
            }
            
            return data;
        } catch (error) {
            console.error(`[Indux Collections] Failed to load API collection:`, error);
            // Return empty array/object to prevent breaking the UI
            return Array.isArray(collection.defaultValue) ? collection.defaultValue : (collection.defaultValue || []);
        }
    }
    ...
    async function loadCollection(collectionName, locale = 'en') {
        ...
        const loadPromise = (async () => {
            try {
                ...
                const collection = manifest.data[collectionName];
                if (!collection) {
                ...
                // Auto-detect collection type based on structure
                if (typeof collection === 'string') {
                    ...
                    if (contentType?.includes('application/json') || collection.endsWith('.json')) {
                        data = await response.json();
                    } else if (contentType?.includes('text/yaml') || collection.endsWith('.yaml') || collection.endsWith('.yml')) {
                    ...
                    data = await loadFromAPI(collection);
                } else if (collection[locale]) {
                    // Localized collection
                    const localizedCollection = collection[locale];
                    if (typeof localizedCollection === 'string') {
                        // Localized local file
                        const response = await fetch(localizedCollection);
                        const contentType = response.headers.get('content-type');

                        if (contentType?.includes('application/json') || localizedCollection.endsWith('.json')) {
                            data = await response.json();
                        } else if (contentType?.includes('text/yaml') || localizedCollection.endsWith('.yaml') || localizedCollection.endsWith('.yml')) {
                            const text = await response.text();
                            const yamlLib = await loadYamlLibrary();
                            data = yamlLib.load(text);
                        } else {
                    ...
                    } else if (localizedCollection.url) {
                        // Localized cloud API
                        data = await loadFromAPI(localizedCollection);
                    } else {
                        console.warn(`[Indux Collections] No valid source found for collection "${collectionName}" in locale "${locale}"`);
                        return null;
                    }
                } else {
                    console.warn(`[Indux Collections] No valid source found for collection "${collectionName}"`);
                    return null;
                }

                // Enhance data with metadata
                let enhancedData;
                const sourceType = typeof collection === 'string' ? 'local' : 'api';
                const sourcePath = typeof collection === 'string' ? collection : collection.url;
