...
function initializePlugin() {
    ...
    async function registerComponents() {
        ...
        if (!window.manifest) {
            console.error('[Indux] Manifest not found');
            return;
        }
    ...
    async function loadComponent(name) {
        ...
        if (!path) {
            console.error(`[Indux] No path found for component: ${name}`);
            return null;
        }
        ...
        try {
            ...
            console.error(`[Indux] Error loading component ${name}:`, error);
            return null;
        }
    }
    ...
    async function processComponent(element) {
        ...
        if (!store.registered.has(name)) {
            console.warn(`[Indux] Component not registered: ${name}`);
            return;
        }
        ...
        if (!content) {
            console.error(`[Indux] Failed to load content for: ${name}`);
        ...
        if (topLevelElements.length === 0) {
            console.error(`[Indux] No content in component: ${name}`);
    ...
    async function initializeComponentsPlugin() {
        ...
        try {
            ...
            console.error('[Indux] Failed to initialize:', error);
        }
    }
